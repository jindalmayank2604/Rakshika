...
export const apiService = {
  ...
  async getReports(filters = {}) {
    ...
    throw new Error(json.message || 'Failed to fetch community reports from backend server.');
  },

  async getIncidents(filters = {}) {
    const params = new URLSearchParams(filters).toString();
    const res = await fetch(`${API_BASE}/incidents${params ? `?${params}` : ''}`, {
      headers: getAuthHeaders(),
      credentials: 'include'
    });
    const json = await res.json();
    ...
    throw new Error(json.message || 'Failed to fetch community incidents from backend server.');
  },
  ...
  async createReport(reportData) {
    ...
    throw new Error(data.message || 'Failed to submit report to backend database.');
  },
